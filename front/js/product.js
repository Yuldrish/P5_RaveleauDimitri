...
function getPost(produit) {
  ...
  btn_envoiAuPanier.addEventListener("click", () => addToCart(produit));
}
...
function addToCart(produit) {
   ...
   */
  if (
    quantiterChoisie.value > 0 &&
    quantiterChoisie.value <= 100 &&
    ...
    const confirmationPopup = () => {
      if (
        window.confirm(`Votre comande de ${choixQuantiter} ${produit.name} ${choixCouleur} à été ajouter au panier, 
    ...
    if (produitLocalStorage) {
      ...
      if (resultatPanier) {
        let nouvelleQuantiter =
          parseInt(optionsProduit.quantiterProduit) +
          parseInt(resultatPanier.quantiterProduit);
